...
__global__ void multiply_vectors(double *a, double *b, double *c)
{
...
void serial_multiply(double *a, double *b, double *c, int n)
{
    for(int i = 0; i < n; i++)
    {
        c[i] = a[i] * b[i];
...
int main()
{
    ...
    multiply_vectors<<< blk_in_grid_1, thr_per_blk_1 >>>(d_A, d_B, d_C);
    ...
    for(int i=0; i<N; i++)
    {
        if(fabs(C[i] - 6.0) > tolerance)
        { 
            printf("error: value of c[%d] = %f instead of 6.0\n", i, C[i]);
            error_1 = 1;
            break;
        }
    }
    ...
    multiply_vectors<<< blk_in_grid_2, thr_per_blk_2 >>>(d_A, d_B, d_C);
    ...
    multiply_vectors<<< blk_in_grid_3, thr_per_blk_3 >>>(d_A, d_B, d_C);
    ...
    printf("\n===========================================\n");
    printf("vector multiplication complete\n");
